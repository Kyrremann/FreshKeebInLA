...
function Asetniop:update(nextLetter)
...
function Asetniop:findHomerow(letter)
   for i=1,#self.homerow do
      local homerow = self.homerow[i]
      if Keyboard.contains(letter, self[homerow]) then
	 return homerow
      end
   end
end
